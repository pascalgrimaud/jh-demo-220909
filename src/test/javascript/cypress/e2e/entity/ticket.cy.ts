...
describe('Ticket e2e test', () => {
  ...
  const ticketSample = { title: 'Frozen' };
  ...
  describe('new Ticket page', () => {
    ...
    it('should create an instance of Ticket', () => {
      cy.get(`[data-cy="title"]`).type('Stand-alone Metal Bedfordshire').should('have.value', 'Stand-alone Metal Bedfordshire');

      cy.get(`[data-cy="description"]`).type('Representative').should('have.value', 'Representative');

      cy.get(`[data-cy="dueDate"]`).type('2022-09-08').blur().should('have.value', '2022-09-08');

      cy.get(`[data-cy="date"]`).type('2022-09-09T04:27').blur().should('have.value', '2022-09-09T04:27');
